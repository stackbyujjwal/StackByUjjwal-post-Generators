...
function updateFont(val) { document.getElementById('card-container').style.fontFamily = val; }
function updateBgColor(val) { document.getElementById('post-canvas').style.backgroundColor = val; }
function updateTheme(val) { document.documentElement.style.setProperty('--theme-color', val); }
...
function executeDownload() {
    ...
    setTimeout(() => {
        html2canvas(canvas, { scale: scaleFactor, useCORS: true, backgroundColor: null }).then(c => {
            ...
            link.download = `Quote-Post-${Date.now()}.${format}`;
