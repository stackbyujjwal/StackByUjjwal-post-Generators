...
function updateTheme(val) { 
    document.getElementById('display-mark').style.color = val;
    document.querySelectorAll('.option-num').forEach(el => el.style.color = val);
}

// Shrink option updates base EM size dynamically
function updateContentScale(scaleValue) {
    document.getElementById('post-canvas').style.setProperty('--base-size', (16 * scaleValue) + 'px');
}

// Modal Logic: Moves canvas inside modal for Side-by-Side preview
...
function executeDownload() {
    ...
    setTimeout(() => {
        html2canvas(canvas, { scale: scaleFactor, useCORS: true, backgroundColor: null }).then(c => {
            ...
            link.download = `Quiz-Post-${Date.now()}.${format}`;
